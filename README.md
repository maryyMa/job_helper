...
- 目标岗位JD：用户输入或指定公司+岗位
- **JD保存规则**：当用户提供JD时，自动保存到 `target-job/{目标公司}-{目标岗位}.md`
...
- 个人简介精简为 **2句话**（职业定位+核心成就）
- 核心数据直接删除或融入具体描述

### 工作经历筛选原则
- **按匹配度筛选**：与岗位匹配的经历重点展示，无关经历一句话带过
- 不按时间顺序筛选，而是按与目标岗位的相关性

### 模块设计原则
- 量化数据融入工作经历和项目经验的具体描述中
- 技能/工具用括号标注核心能力（如 `Coze(工作流)、OpenCode(Agent/Skill)`）
- 每个bullet point以动词开头，量化数据融入描述不堆砌
